...
<x-page-head>
  <a href="<?= site_url(ADMIN_AREA . '/galleries') ?>" class="back">&larr; <?= lang('Galleries.galleryTitle') ?></a>
  <h2><?= isset($gallery) ? lang('Galleries.editGallery') : lang('Galleries.newGallery') ?></h2>
</x-page-head>

<?php if (isset($gallery) && $gallery->deleted_at !== null) : ?>
  <div class="alert danger">
    <?= lang('Galleries.galleryWasDeleted') . ' ' . CodeIgniter\I18n\Time::parse($gallery->deleted_at)->humanize() ?>.
    <a href="<?= url_to('recycler-restore', 'galleries', $gallery->id) ?>"><?= lang('Galleries.restoreGallery') ?></a>
  </div>
<?php endif ?>
...
<x-admin-box>
    ...
    <div class="row" id="types">
      ...
      <?php if (isset($gallery->type)) : ?>
        <?php if ($gallery->type == 'IMAGE') : ?>
          <?= $this->include('App\Modules\Galleries\Views\_type_image') ?>
        <?php else :  ?>
          <?= $this->include('App\Modules\Galleries\Views\_type_video') ?>
        <?php endif  ?>
      <?php else : ?>
        <?= $this->include('App\Modules\Galleries\Views\_type_image') ?>
    </div>
  ...
  <div class="text-end py-3">
    <input type="submit" value="<?= lang('Galleries.saveGallery') ?>" class="btn btn-primary btn-lg">
  </div>
